...
VALUES 
   (2, 100000538, 900001624, 1, 1, 2, 1, '99020015', '88882024', '02/02/2024', 
   '05/02/2024', '10/02/2024', 2, 100, 10000, '2o Contrato de Conserva para Teste');
   
-- Recupera Id do ultimo conrato Inserido
SELECT max(ctr.id) AS id_contrato_inserido -- 3072 e 3073
...
SELECT ctr.id                        AS id_ctr,
 ...
 WHERE ctr.tipo      = 2 -- 2-Contrato de Conserva
   ...
   AND ctr.id        = 2769; -- id do Contrato
...
SELECT -- Relação de Ids de integração
 ...
 WHERE med.id = 2019; -- id da Medição inserida
...
UPDATE controle_conservacao.medicoes 
 ...
 WHERE id = 2019; -- id da Medição inserida
...
UPDATE controle_conservacao.medicoes 
 ...
 WHERE id = 2019;  -- id da Medição com assinaturas finalizadas
...
UPDATE controle_conservacao.medicoes 
 ...
 WHERE id = 2019; -- id da medição Liberada para Protocolar
