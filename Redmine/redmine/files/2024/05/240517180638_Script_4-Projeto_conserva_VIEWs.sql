...
LEFT JOIN controle_conservacao.medicoes_fiscais     mcf ON med.id = mcf.id_medicao 
                                                                AND mcf.id_tipo_fiscal = 2 -- 2= 1o Membro da Comissão                                                                
              ...
              SELECT 
               ...
               (RIGHT(pat.ano_mes,2)::INTEGER -RIGHT(pan.ano_mes,2)::INTEGER) 
                ...
                CASE WHEN pan.id = pat.id           THEN 1 -- SE já existir uma medição no periodo vigente ENTAO 1-Med.Aberta
                     ...
                     WHEN COALESCE((SELECT count(ctf.id) 
                                      FROM controle_conservacao.contratos_fiscais ctf 
                                     WHERE ctf.id_contrato = ctr.id),0) = 0 THEN 4  -- SE Contrato Não Possui Comissão de ficalização ENTAO 4-Impossivel Abrir
                ...
                CASE WHEN pan.id = pat.id           THEN 'Medição Aberta no Período Vigênte. (Status Atual = '      || stm.descricao || ')' 
                     WHEN ctr.id_status        <> 2 THEN 'Contrato Não Está Vigênte. (Status Atual = '              || stc.descricao || ')' 
                     WHEN med.id_status_fisico <> 7 THEN 'Medição Anterior Ainda Não Foi Fechada. (Status Atual = ' || stm.descricao || ')'                    
                     WHEN COALESCE((SELECT count(ctf.id) 
                                      FROM controle_conservacao.contratos_fiscais ctf 
                                     WHERE ctf.id_contrato = ctr.id),0) = 0 THEN 'Comissão de Fiscalização do Contrato: [' || ctr.nr_contrato_der || '] Ainda não foi Cadastrada.'
                     WHEN (((LEFT(pat.ano_mes,4)::INTEGER - LEFT(pan.ano_mes,4)::INTEGER) * 12) +
                           (RIGHT(pat.ano_mes,2)::INTEGER - RIGHT(pan.ano_mes,2)::INTEGER)) > 1 
...
WHERE med.id_status_fisico IN (3,4); -- Aguardando 3-Assinatura 4-Conferencia
-- ------------------------------------------------------------------------------  
...
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.valor_medido        IS '(A) Valor medido a ser informaado pelo usuário.';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.valor_ref_glosa     IS '(B) Valor Referente a Glosa de medições anteriores.';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.fator_reajuste      IS '(X) Fator de reajuste aplicado ao Valor PI.';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.valor_reajuste      IS '(C) = (A * X)';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.valor_ajuste_contratual  IS '(D) Valor de ajuste contratual eventual';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.valor_processo      IS '(E) = (A + B + C + D)';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.descricao_glosa     IS 'Descrição do valor informado no item (B).';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.descricao_ajuste_contratual  IS 'Descrição do valor informado no item (D).';  
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.nr_protocolo        IS 'NUP gerado no sistema SUITE';
COMMENT ON COLUMN controle_conservacao.vw_medicoes_aguardando_administrativo.data_hora_protocolo IS 'Data e Hora que goi gerado o NUP no SUITE';
...
LEFT JOIN controle_conservacao.medicoes_fiscais     mcf ON med.id = mcf.id_medicao AND mcf.id_tipo_fiscal = 2 -- 2- 1o Membro da Comissão                                                                
LEFT JOIN der_ce_coorporativo.vw_funcionario                 fis ON fis.matricula = mcf.id_fiscal;
...
--------------------------------------------------------------------------------
-- VIEW....: controle_conservacao.vw_quadro_financeiro_contrato_conserva
-- CLASSE..: controle_conservacao.VoQuadroFinanceiroContratoConserva.java
-- Objetivo: 
--------------------------------------------------------------------------------
CREATE OR REPLACE VIEW controle_conservacao.vw_quadro_financeiro_contrato_conserva AS
SELECT med.id,
       med.id_contrato,
       ctr.objeto            AS contrato_objeto,
       -- Identificação do contrato
       ctr.nr_contrato_der   AS contrato_nr_der,
       ctr.nr_contrato_ext   AS contrato_nr_cliente,
       ctr.nr_contrato_sic   AS contrato_nr_sic,
       ctr.nr_licitacao      AS contrato_nr_licitacao,
       ctr.nr_ordem_servico  AS contrato_nr_os,
       -- Dados Gerais do Contrato
       'Conserva Rodoviaria'::TEXT AS contrato_tipo,
       dop.nome              AS contrato_distrito,
       ctd.nome_fantansia    AS contrato_contratada,
       ges.nome_referencia   AS contrato_gestor,
       sct.descricao         AS contrato_status,
       -- Datas relevantes do Contrato
       to_char(ctr.data_proposta    ,'dd/MM/yyyy') AS contrato_data_proposta,
       to_char(ctr.data_assinatura  ,'dd/MM/yyyy') AS contrato_data_assinatura,
       to_char(ctr.data_publicacao  ,'dd/MM/yyyy') AS contrato_data_publicacao,
       to_char(ctr.data_fim_execucao,'dd/MM/yyyy') AS contrato_data_fim_execucao,
       to_char(ctr.data_fim_vigencia,'dd/MM/yyyy') AS contrato_data_fim_vigencia,
       -- Valores Globais do Contrato
       ctr.valor_original    AS contrato_valor_original,
       ctr.total_aditivo     AS contrato_total_aditivo,
       ctr.valor_pi          AS contrato_valor_pi,
       ctr.total_reajuste    AS contrato_total_reajuste,
       ctr.valor_atual       AS contrato_valor_atual,
       -- Dados Medição
       med.nr_medicao                       AS medicao_nr,
       SUBSTRING(pmo.ano_mes, 5, 2)         || '-'   || 
       SUBSTRING(pmo.ano_mes, 1, 4)         AS medicao_periodo_mes_ano,
       to_char(med.data_inicio, 'DD/MM/YY') || ' a ' || 
       to_char(med.data_fim,    'DD/MM/YY') AS medicao_periodo_datas,
       med.nr_protocolo                     AS medicao_protocolo,
       COALESCE(sme.sigla, '')::VARCHAR(3)  AS medicao_status_financeiro,
       -- Valores Medição       
       COALESCE(med.valor_medido   ,0) + 
       COALESCE(med.valor_ref_glosa,0)         AS medicao_valor_medido_pi,
       COALESCE(med.valor_reajuste ,0) + 
       COALESCE(med.valor_ajuste_contratual,0) AS medicao_valor_adicional,
       COALESCE(med.valor_processo ,0)         AS medicao_valor_processo,
       -- Valores Financeiros Até a Medição Corrente
       COALESCE(des.total_empenhado,0)         AS medicao_total_empenhado,
       COALESCE(des.total_pago     ,0)         AS medicao_total_pago,
       COALESCE(med.valor_processo ,0) - 
       COALESCE(des.total_pago     ,0)         AS medicao_saldo_a_pagar,
       -- Valores Acumulado ate Medição Vigente
       mac.acumulado_medido   + 
       mac.acumulado_ref_glosa   AS medicao_acumulado_valor_pi,
       mac.acumulado_reajuste + 
       mac.acumulado_ajuste      AS medicao_acumulado_valor_adicional,
       mac.acumulado_protocolado AS medicao_acumulado_valor_protocolado
 FROM der_ce_contrato.contratos                      ctr
 JOIN der_ce_contrato.status_contrato                sct ON sct.id = ctr.id_status
 JOIN der_ce_coorporativo.pessoas_juridicas          ctd ON ctd.id_pessoa = ctr.id_contratada
 JOIN der_ce_coorporativo.vw_funcionario             ges ON ges.id = ctr.id_gestor
 JOIN der_ce_coorporativo.distritos_operacionais     dop ON dop.id = ctr.id_distrito
 JOIN controle_conservacao.medicoes                  med ON ctr.id = med.id_contrato
 JOIN controle_conservacao.status_medicao_financeiro sme ON sme.id = med.id_status_financeiro
 JOIN der_ce_medicao.periodos_medicoes_obras         pmo ON pmo.id = med.id_periodo
 LEFT JOIN der_ce_financeiro_despesa.despesas        des ON des.id = med.id_despesa
 LEFT JOIN LATERAL -- Recupera Valores Acumulado por Medição
     (SELECT sum(mda.valor_medido)            AS acumulado_medido,
             sum(mda.valor_ref_glosa)         AS acumulado_ref_glosa,
             sum(mda.valor_reajuste)          AS acumulado_reajuste,
             sum(mda.valor_ajuste_contratual) AS acumulado_ajuste,
             sum(mda.valor_processo)          AS acumulado_protocolado
        FROM controle_conservacao.medicoes mda
       WHERE mda.id_contrato = med.id_contrato
         AND mda.nr_medicao <= med.nr_medicao
         AND mda.id_status_fisico > 2 -- Nao Entra 1-Aberta e 2-Aguardando Validação
     ) mac ON TRUE        
ORDER BY med.id_contrato, med.nr_medicao;